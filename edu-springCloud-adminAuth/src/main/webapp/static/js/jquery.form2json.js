    ...
    $.fn.form2json = function(options) {
        ...
        multiVal.each(function() {
            ...
            if (key && (val || settings.allowEmptyMultiVal)) {
                var keyHierarchy = key.split('.'),
                    dKeys = $.map(keyHierarchy, function(k) {
                        return $.isFunction(settings.keyTransform) ? settings.keyTransform(k.replace(/\[[0-9]*\]/i, ""), item) : k.replace(/\[[0-9]*\]/i, "");
                    }),
                ...
                if (obj[prop]) {
                    ...
                    if(settings.checkOneUseArray&&"checkbox"==item[0].type.toLowerCase()){
                        obj[prop] = [val];
                    }else {
                        obj[prop] = val;
                    }
